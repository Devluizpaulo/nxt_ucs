...
import { Search, FileText, Trash2, ChevronLeft, ChevronRight, Building2, Plus, Layers } from "lucide-react";
...
export default function AssociacoesPage() {
  ...
  const handleBulkImport = async (data: any[]) => {
    ...
    const colRef = collection(firestore, "associacoes");
    ...
    <div className="flex min-h-screen bg-[#F8FAFC]">
      ...
      <main className="flex-1 flex flex-col">
        <header className="h-20 bg-white/50 backdrop-blur-md px-8 flex items-center justify-between border-b border-slate-200 sticky top-0 z-10 print:hidden">
          <h1 className="text-xl font-medium text-slate-600">Auditoria de Saldos: <span className="font-bold text-slate-900">Associações</span></h1>
          <div className="flex items-center gap-6">
            ...
            <div className="w-10 h-10 bg-indigo-600 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-md">AD</div>
          </div>
        </header>
            ...
            <Tabs value={activeTab} onValueChange={v => setActiveTab(v as EntityStatus)} className="w-full">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex gap-3">
                  ...
                  <EntityBulkImport onImport={handleBulkImport} type="associacao" />
